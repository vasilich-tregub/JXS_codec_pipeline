...
int main()
{
    ...
    for (int i = 0; i < im.width * im.height; ++i)
    {
        int rs = (1 << im.depth) / 256; // right shift, bitdepth - 8 bit positions
        buf[i] =
            (uint8_t)((im2[i] >> s) / rs) * 256 * 256 + // red
            (uint8_t)((im1[i] >> s) / rs) * 256 + // green
            (uint8_t)((im0[i] >> s) / rs); // blue
    }
    ...
    int stride = im.width;
    int rs = 1 << (im.depth - 1); // right shift, bitdepth positions - 1
    ...
    for (int i = 0; i < im.width * im.height; ++i)
    {
        int rs = (1 << im.depth) / 256; // right shift, bitdepth - 8 positions
        buf[i] =
