...
{
    const int WIDTH = 320; // 1920; // 3840;
    const int HEIGHT = 240; // 1080; // 2160;
...
int image_paint(xs_image_t& image)
{
    for (int iy = 0; iy < image.height; ++iy)
    {
        ...
        {
            image.comps_array[0][iy * image.width + ix] = (uint8_t)((iy * 6 / 10 - ix * 3 * 6 / 20 + 40)); // bands of const color intensity along hyperbolas
    ...
    for (int iy = 0; iy < image.height; ++iy)
    {
        for (int ix = 0; ix < image.width; ++ix)
        {
            image.comps_array[1][iy * image.width + ix] = (uint8_t)(iy * ix * 6 * 6 / 1600); // bands of const color intensity along hyperbolas
            // = ((iy * 8 + ix * 24 / 4)); //((iy % 32) > 16) ? 0 : 255; gives straight // straight 45 degree skew bands 
        }
    }
    for (int iy = 0; iy < image.height; ++iy)
    {
        for (int ix = 0; ix < image.width; ++ix)
        {
            image.comps_array[2][iy * image.width + ix] = (uint8_t)((iy  * 6 / 10 + ix * 3 * 6 / 20)); //(iy * ix / 64); // bands of const color intensity along hyperbolas
